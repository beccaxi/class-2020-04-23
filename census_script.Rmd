...
raw <- read_csv("censoc_dmf_v1/censoc_dmf_v1/censoc_dmf_v1.csv", col_types = cols())
...
# Bad data?? bmonth = 0 (supposed to start at 1)

raw <- raw %>% 
  filter(bmonth != 0)

# more than 5.7 million men

raw <- raw %>% 
  mutate(age = dyear - byear) %>% 
  arrange(desc(age))

# oldest man was 123 years old, born in 1860

raw %>% 
  group_by(byear) %>% 
  summarize(mean_age = mean(age)) 

# 1854 and 1858 have the top two average ages at time of death...perhaps those
# born in the years leading up to WWI would have lowest average age
...
# January, December, March, and February have the highest number of deaths
```
...
x <- raw %>% 
  mutate(death_date = paste(dyear, dmonth, "01", sep = "-")) %>% 
  mutate(death_date = ymd(death_date)) %>% 
  select(death_date, age)
...
plot <- deaths %>% 
  ggplot(aes(x = age, y = deaths)) +
    ...
    labs(
      ...
      subtitle = "Ages at deaths in normalesque...",
      x = "Age at Death",
      y = "Total Number",
...
# slightly skewed to the left - relationship between retirement and chance of death
```
...
seasons <- small %>% 
  ...
  mutate(season = ifelse(dmonth %in% c(10, 11, 12, 1, 2, 3), "Cold", "Warm")) %>% 
  group_by(death_date, season) %>% 
  summarize(deaths = n())

seasons_plot <- seasons %>% 
  ggplot(aes(x = death_date, y = deaths, color = season)) +
    geom_point() +
    geom_line() +
